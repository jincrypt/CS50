...
def index():
    ...
    stocks = db.execute("SELECT symbol, name,  SUM(shares) AS 'shares', SUM(shares * price) / SUM(shares) AS 'price', SUM(shares * price) AS 'total' FROM transactions WHERE user_id = :id GROUP BY symbol", 
                        id=session["user_id"])
                        
    cash = db.execute("SELECT cash AS 'cash' FROM users WHERE id =:id", 
                      id=session["user_id"])
